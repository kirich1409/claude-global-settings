...
Чем закрыты классы каналов из `~/.claude/references/external-sources.md` на этих машинах. Читать по
указателю из `CLAUDE.md` на любом вопросе о JVM-библиотеке, в том числе вне Gradle-проекта:
исследование в пустом каталоге, сравнение библиотек до выбора, ответ по координате без открытого
билд-файла. Набор машинно-зависимый: наличие проверять, отсутствующий канал называть вслух, а не
подменять памятью.
