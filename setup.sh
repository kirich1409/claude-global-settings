...
echo "Old settings.json saved as: $BACKUP_DIR/settings.json"
echo ""
...
if ! grep -q 'alias csync=' "$SHELL_RC" 2>/dev/null; then
  echo 'alias csync="$HOME/.claude/hooks/sync-settings.sh"' >> "$SHELL_RC"
  echo "Added csync alias to $SHELL_RC"
else
  echo "csync alias already exists in $SHELL_RC"
fi
