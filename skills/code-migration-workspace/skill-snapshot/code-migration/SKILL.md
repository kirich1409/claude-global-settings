...
|----|----------|-------------|
| **Preparation** | Module isolation, `migration-checklist.md`, `behavior-spec.md`, KMP dependency audit | Sets up the plan; no behavior change; easy to review |
...
5. **Analyze codebase impact** — gather the facts that drive the proposal:
   ...
   - **KMP dependency compatibility** (KMP migrations only): for every dependency in the affected module(s), verify it publishes a KMP-compatible artifact — a library that works on Android may fail to compile in `commonMain`. Read `build.gradle.kts` / `build.gradle` and audit each dependency:
     - Use `maven-mcp` tools if available (`scan_project_dependencies` then `check_multiple_dependencies`) — they can tell you the latest version and whether it includes KMP metadata
     - If unavailable: search Maven Central for `<group>:<artifact>` and look for Kotlin Multiplatform mentions in the README, changelog, or published artifact metadata
     - Categorize each dependency into one of four groups:
       - **KMP-compatible now** — current version already works in `commonMain`, no action needed
       - **KMP available, patch/minor update** — a newer minor version adds KMP support; bumping the version is low risk
       - **KMP only in breaking major version** — KMP support exists but only in a major version with API changes, package renames, or a migration of its own (e.g., Coil 2→3: `io.coil-kt:coil` → `io.coil-kt.coil3:coil-core`, different API surface). For each of these: use `maven-mcp:dependency-changes` to assess what changed between current and KMP-supporting version, and estimate the migration effort. This is a **nested migration** — it must be planned and scoped as a separate step before or alongside the KMP migration.
       - **No KMP support** — no KMP artifact exists at any version; needs a KMP alternative (e.g., Retrofit → Ktor, Hilt → Koin, Glide → Coil-KMP) or must stay in `androidMain`
     - Present the compatibility matrix to the user before Phase 2. Each "breaking major version" or "no support" entry changes the scope of the migration. The user must decide for each: migrate the dependency as a pre-step (separate PR), include it in scope, stay `androidMain`-only, or drop it. **Do not absorb these decisions silently — they affect effort, risk, and strategy.**
