...
slug: {slug}
date: {YYYY-MM-DD}
status: draft                    # draft | approved | blocked_upstream. Последнее — спека в правке по откату с нижестоящего этапа (~/.claude/references/pipeline-rollback.md): `approved` снимается на входе в правку и возвращается только по PASS/CONDITIONAL ревью, чтобы потребители ниже не читали мутирующий документ как утверждённый.
...
- [ ] **AC-1** — {конкретное наблюдаемое поведение, а не внутреннее состояние} `[user]`
- [ ] **AC-2** — {ещё критерий} `[agent]`
- [ ] **AC-3** — {критерий на ошибку или граничный случай} `[agent]`
- [ ] **AC-4** — {критерий производительности с конкретными числами, если уместен} `[research]`
- [ ] **AC-5** — {критерий совместимости, если уместен} `[agent]`

**Провенанс** — обязательная метка в конце каждого критерия: `[user]` — пользователь потребовал
явно, своими словами; `[agent]` — сформулировал агент (синтез, дефолт, вывод из чеклиста);
`[research]` — следует из находки research (в скобках указать её). Метку читает откат по конвейеру
(`~/.claude/references/pipeline-rollback.md`): критерий `[agent]`, оказавшийся невыполнимым,
перерешается автономно, `[user]` — только блокирующим вопросом. Неоднозначный случай метится
`[user]` — консервативная сторона ошибки.
...
|---|---|---|---|
| {что решали} | {что выбрали} | {почему это, а не альтернативы} | user / agent / research |

`Source` — кто решение принял: `user` — пользователь выбрал в интервью или потребовал сам; `agent` —
выбрал движок спеки (в том числе дефолтом, когда пользователь не выбирал); `research` — предопределено
находкой research. Колонка не бюрократия: по ней откат по конвейеру определяет, можно ли перерешить
требование автономно (`~/.claude/references/pipeline-rollback.md`). Не заполнена — считается `user`.

## Rollback History

*(Раздела нет, пока не было ни одного отката. Заводит и ведёт только `write-spec --rollback`.)*

Единственный источник истины по числу кругов отката: файлы состояния эфемерны и у каждого инициатора
свои, спека же постоянна и всегда является целью отката. Инициаторы (`write-plan`, `/acceptance`)
раздел читают, пишет в него только `write-spec`.

| Тема | Круг | Класс | Инициатор | Что изменилось |
|---|---|---|---|---|
| {область или дефект-класс, не дословная формулировка} | 1 | infeasible / contradictory / underspecified | write-plan ф.1.5 / гейт / acceptance | {как изменилось требование, либо `refuted` — откат отклонён, требование подтверждено} |
