...
**Upstream block:** <класс> / <адрес в вышестоящем документе> / круг N — либо `none`
**Date:** <дата>
...
- **FAILED** с непустым `Upstream block` → в код **не** возвращаться: дефект введён выше, и цикл
  фиксов здесь только наращивает обходные пути вокруг требования, которое отменяют. Откат по
  `~/.claude/references/pipeline-rollback.md`, PR — по его шагу 4.5. Приёмка перезапускается после
  того, как вышестоящий артефакт переутверждён, а план прошёл свой гейт на изменённом входе.
