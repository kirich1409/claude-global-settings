...
**BLOCK, который не чинится в коде**, — требование спеки невыполнимо, два AC противоречат друг другу,
AC не определяет поведение, которое реализация не вправе выбрать сама, — не крутится циклом фиксов:
это upstream-дефект, и цикл здесь только наращивает обходные пути. Прекратить цикл, пометить находку
классом (`infeasible` / `contradictory` / `underspecified`) и вести по
`~/.claude/references/pipeline-rollback.md` — с верификацией до отката и провенансом требования,
решающим, автономна ли правка спеки.

Здесь у агента максимальный стимул объявить AC невыполнимым — код уже написан и падает, — поэтому
планка та же, что и выше по конвейеру, и повторяется явно:

- **Падающая реализация доказательством невыполнимости не является.** Нужно отсутствие API в
  разрешённой версии, запрет платформы, замеренный недостижимый бюджет либо процитированная пара
  взаимоисключающих AC — и подтверждение `spike-runner` / `hard-case-arbiter`. Всё остальное значит,
  что чинить надо код.
- **Пакетом:** все upstream-находки раунда уходят наверх одним заходом. Круг отката читается из
  раздела `## Rollback History` спеки — тема уже откатывалась, значит автономного круга нет.
- **Статус:** сводный остаётся `FAILED`, а поле `Upstream block` расписки заполняется классом,
  адресом и кругом — по нему потребители отличают этот случай от обычного `FAILED` и не гонят работу
  обратно в код. После переутверждения спеки план проходит свой гейт заново (спека изменилась —
  значит изменился и вход плана), и лишь затем приёмка перезапускается.
- **PR:** черновой PR к этому моменту уже открыт (`~/.claude/rules/workflow.md`), поэтому откат
  затрагивает его всегда. Промоция запрещена, тело помечается блоком отката, ready-PR возвращается в
  черновик, начатое ревью получает комментарий — таблица состояний в
  `~/.claude/references/pipeline-rollback.md`, шаг 4.5. Часть работы, успевшая уехать в `main`,
  откатом не отменяется и разбирается там же.
