...
неразрешимый rebase, исчерпанный `--max-rounds`, **upstream-дефект в артефактах конвейера**. Это
блокеры, а не гейты: они означают, что дальше нужен человек, а не что скилл ждёт разрешения
продолжить.
...
**Гейт отката — до всего остального в раунде.** Прочитать артефакты конвейера по слагу ветки:
`docs/plans/<slug>/plan.md` (`review_verdict`), спеку по его `spec:` (`status`), расписку приёмки
`swarm-report/<slug>-acceptance.md` (`Upstream block`). Любое значение `blocked_upstream` либо
непустой `Upstream block` — раунд не начинать: дефект введён выше по конвейеру, а всё, что умеет цикл
— фиксы, перезапрос ревью, промоция, слияние, — делает работу поверх требования, которое сейчас
отменяют. Действие: обработать PR по `~/.claude/references/pipeline-rollback.md`, шаг 4.5 (вернуть в
черновик, пометить тело, при начатом ревью — комментарий), записать терминальное состояние `blocked`
с причиной `upstream_defect` и выйти. `--merge=auto` этот гейт не снимает: он управляет
подтверждением слияния, а не правом сливать. Возобновление — новым вызовом скилла после того, как
вышестоящий артефакт переутверждён, а нижестоящий прошёл свой гейт заново.
...
| `blocked` | поднят блокер: защита от петли падений, несовпадение целостности, неразрешимый rebase, `BEHIND` при `--rebase=no`, DISCUSSION требует пользователя, опросы или `--max-rounds` исчерпали лимит, пользователь сказал «stop», upstream-дефект (`review_verdict: blocked_upstream` у плана, `status: blocked_upstream` у спеки либо непустой `Upstream block` в расписке приёмки) | в файле состояния заполнено `Blockers raised` (в том числе причина `"user stop"`, когда применимо), сообщение в сессии объясняет следующий шаг |
