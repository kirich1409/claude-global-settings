...
2. **Board sync (best-effort).** If the PR references an issue (`Closes #N` / `Fixes #N` / `Resolves #N`) and the repo has a Projects v2 board, move the card: `$HOME/.claude/scripts/gh/transition_status.sh <N> done`. No linked issue or no board → skip silently, the script already degrades gracefully. See `skills/drive-to-merge/references/tracker-ops.md` § The board.
