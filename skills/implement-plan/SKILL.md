...
- **`review_verdict: blocked_upstream`** → план заблокирован дефектом спеки или research и ждёт
  отката (`~/.claude/references/pipeline-rollback.md`); реализовывать его нельзя тем более: он писался
  под требование, признанное невыполнимым. Показать `upstream_blocker` (это путь к файлу состояния с
  пакетом находок) и остановиться.
- **`review_verdict: overridden_by_user`** → ревью пройдено не было, пользователь продолжил своим
  решением; `status` при этом остаётся `draft`. Исполнять можно, но показав дословную причину
  оверрайда: реализация идёт по неотревьюенному плану, и потребители ниже должны это знать.
...
Делегировать это — верификатору или general-purpose субагенту, в фоне при
