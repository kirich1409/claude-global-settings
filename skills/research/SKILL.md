...
агентов одновременно — по одному на трек, — каждый исследует свой срез независимо, затем оркестратор сводит находки в один
...
**Режим определяется до всего остального.** `auto` включается принудительно, без учёта аргументов, там, где диалога физически нет: субагент, фоновый или headless-запуск, cron. `AskUserQuestion` оттуда до пользователя не доходит, и попытка спросить превращается в тихий простой. Вызов из другого скилла интерактивной сессией — случай отдельный: диалог там технически возможен, но конфигурационный раунд посреди чужого конвейера сбивает его темп, поэтому режим по умолчанию тоже `auto`, а вызывающий скилл может затребовать `--mode=ask` явно.
