...
| `--tracks=<список>` | Запустить ровно эти треки: `codebase`, `web`, `docs`, `deps`, `oss`, `architecture`. Не задан — отобрать самому по таблице ниже. Явный список отменяет отбор, но не отменяет ни правило минимума в два трека, ни требование раздельных экземпляров `source-researcher` на каждый внешний класс. |
...
**Исключение — точечный перезапрос по откату.** Трек, запрошенный из `write-spec` при
`upstream_defect` (находка research оказалась ложной), обязан обновить раздел `## Findings`
существующего `swarm-report/research/research-<slug>.md`: опровергнутая находка помечается с
доказательством. Новый отчёт при этом не создаётся. Правило выше запрещает плодить отчёты, а здесь
отчёт уже есть и его читают потребители ниже по конвейеру — оставленная в нём опровергнутая находка
переедет в следующую спеку.
