---
name: research
argument-hint: "<тема> [--tracks=<codebase,web,docs,deps,oss,architecture>] [--focus=<акценты>] [--oss-repos=<owner/repo,…>] [--autonomous]"
description: >
...
| `--oss-repos=<owner/repo,…>` | Репозитории, с которых начинает трек OSS Examples. Заданы — разведка кандидатов и вопрос про них в фазе 1.6 не выполняются. |
...
`--oss-repos` — вопрос про референсные репозитории, `--autonomous` — весь диалог целиком. Скилл, вызванный без аргументов, обязан пройти фазу 1.5, а не
...
конвейера сбивает его темп, поэтому по умолчанию он тоже автономный, а вызывающий скилл может
затребовать конфигурационный раунд явно.
...
или добавленный пользователем в 1.5. Пропускается при `--oss-repos` (репозитории названы), при
`--autonomous` и в неинтерактивном вызове. Случай перенаправления по правилу минимума на одиночный
`source-researcher` с `focus: oss-examples` — тот же: фаза проходится и перед ним, репозитории идут
в его промпт.
...
- OSS seeds: {репозитории из фазы 1.6 и --oss-repos; «—», если трек не запущен или разведка пуста}
...
- **Major/critical, закрывается исследованием** → перезапустить нужный трек тем же промптом, что в
  фазе 2, включая блок акцентов, если он был, и отревьюить заново.
