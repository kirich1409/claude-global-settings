...
| `--tracks=<список>` | Запустить ровно эти треки: `codebase`, `web`, `docs`, `deps`, `oss`, `architecture`. Не задан — отобрать самому по таблице ниже и предъявить отбор в фазе 1.5. Явный список отменяет и отбор, и вопрос про состав, но не отменяет ни правило минимума в два трека, ни требование раздельных экземпляров `source-researcher` на каждый внешний класс. |
...
Автономный режим включается принудительно, без учёта аргументов, там, где диалога физически нет:
субагент, фоновый или headless-запуск, cron. `AskUserQuestion` оттуда до пользователя не доходит, и
попытка спросить превращается в тихий простой. Вызов из другого скилла интерактивной сессией —
случай отдельный: диалог там технически возможен, но конфигурационный раунд посреди чужого
конвейера сбивает его темп, поэтому он тоже автономный. Способа затребовать раунд из вызывающего
скилла нет намеренно: конфигурация исследования — разговор с пользователем, а он в этот момент
разговаривает с конвейером.
...
перенаправленных тем не создавать. Единственное, что стоит между решением о перенаправлении и
выходом, — фаза 1.6, и только когда единственный трек это OSS Examples: одиночный
`source-researcher` получает референсные репозитории на тех же основаниях, что и трек в консорциуме.

В интерактивном режиме перенаправление не выполняется молча: единственный трек становится первой опцией
...
Дефолт: фаза проходится всегда, кроме `--autonomous` и принудительной автономности неинтерактивного
вызова — там сразу фаза 2. Отдельного флага, включающего раунд, нет: не выключен — значит идёт.
...
| Опция | Чем является |
|---|---|
| 1 (Recommended) | результат автоотбора фазы 1 — то, что запустилось бы под `--autonomous`; в `description` по одной причине на трек. Отбор оставил единственный трек — опцией 1 становится перенаправление на прямой инструмент по таблице правила минимума, а опции 2 и 3 расширяют состав до консорциума |
...
задан — вопрос 1 не задавать: состав уже назван. Назван состав из одного трека — идёт
перенаправление по таблице правила минимума, а не опция раунда: показывать выбор тому, кто уже
выбрал, незачем. Перенаправление отменяет фазу 1.5 целиком, вместе с вопросом про акценты, —
консорциума, которому они адресованы, не будет.
...
ввод, оставивший один трек, уходит в перенаправление по таблице правила минимума. Названное в
...
получены одним вызовом. Треков после переноса не осталось — вернуться к опции 1, остался один — к его
перенаправлению.
...
`--autonomous` и неинтерактивный вызов снимают только вопрос: диалога там нет, а разведка — не
...
- **Major/critical, закрывается исследованием** → перезапустить нужный трек тем же промптом, что в
  фазе 2, включая блок акцентов и, для OSS Examples, блок `Seed repositories`, если они были, и
  отревьюить заново.
