...
description: >
  ...
  написание инженерному агенту, проверяет результат. Плюс режимы red-green для регрессии на баг и
  характеризации legacy без спеки. Triggers: "write tests for", "add tests to", "increase
  coverage", "add a regression test", «покрой тестами», «здесь нет тестов». НЕ для: документа
  тест-плана (/write-plan --test-plan), прогона проверок на живом приложении (/acceptance),
