...
| Кастомные агенты (`~/.claude/agents/`, 18) | `~/.cursor/agents/` | symlink `~/.cursor/agents` → `~/.claude/agents` |
...
- Агенты в формате Claude Code (лишние frontmatter-поля `tools:`/`model:`) Cursor загружает, игнорируя незнакомые поля. Проверено: `/architecture-expert` маршрутизируется.
...
Создаёт symlink'и `~/AGENTS.md`, `~/.cursor/agents`, `~/.cursor/skills/rules-*`. Идемпотентно; реальные (не-symlink) файлы не затирает. Запускать вне активной Claude-сессии.
...
- Билд Cursor 6-недельной давности на момент адаптации; на более новых билдах авточтение `~/.claude/agents` на user-level может заработать напрямую (тогда symlink `~/.cursor/agents` избыточен, но безвреден).
