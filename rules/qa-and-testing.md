...
**L5 execution — close it yourself, autonomously.** L5 is not "for the user to run." Always attempt to close L5 on this machine without involving the user: drive the running app via mobile MCP (`mcp__mobile__*`) / `manual-tester` on an emulator/simulator **by default**, escalating to a physical device only when the change critically depends on real-hardware behavior an emulator cannot reproduce (real biometric sensor HAL, camera, NFC, GPS, sensor fusion). First check empirically what is available (`adb devices -l`, `emulator -list-avds`, `xcrun simctl list`) — never declare L5 infeasible from theory. **If a needed emulator / AVD / system image is not installed but is easy to install, install it and run** — "not installed" is not a blocker. Build and install the APK yourself, drive the flow, emulate inputs (e.g. `adb -e emu finger touch <id>`). Involving the user is the **last resort** — permitted only when you have genuinely exhausted every autonomous option and no path is left at all, not merely when it is inconvenient or slower. Legitimate last-resort walls: credentials/secrets you do not have and cannot obtain, a backend reachable only on a closed/VPN network you are not on, or behavior that exists exclusively on physical hardware. Everything else you run yourself, autonomously, before reporting.
