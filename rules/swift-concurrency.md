...
Гарантия `@MainActor` статическая, её держит компилятор: синхронно вызвать такой метод из не-изолированного контекста нельзя вовсе — ошибка `#ActorIsolatedCall`. Из `@MainActor`-контекста вызов синхронный и идёт на main. Не городить `DispatchQueue.main.async`-обёртки и рантайм-проверки «а вдруг не main»: изоляция уже доказана на этапе компиляции.

Единственный способ войти в изоляцию без `await` — `MainActor.assumeIsolated`. Это утверждение, а не приведение: вызов не с main падает в рантайме. Применять только там, где main-поток гарантирован извне (например, колбэк UIKit/AppKit), и никогда — чтобы «обойти» ошибку компилятора.
