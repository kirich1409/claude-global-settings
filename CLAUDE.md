...
- **Артефакт агента по умолчанию неверен, пока минимальный эмпирический верификатор не доказал обратное.** Распространяется на всё, не только код: конфиг → `validate-config`/schema/dry-run; hook → реальный запуск; permission-правила → `validate-config` + warnings свежей сессии. «Валидный JSON», «выглядит правильно» и зелёный diff — не проверка. Исключения gates («конфиг без логики») снимают церемонию `/finalize` и `/acceptance`, но не этот верификатор.
...
- **task-execution.md** — протокол блокирующих ошибок; первопричина над подавлением; scope creep; обработка большого вывода; фоновые команды (`run_in_background` вместо `&`/`nohup`)
...
- **swift-concurrency.md** — неочевидные правила Swift Concurrency: размещение `@MainActor`, антипаттерн Task.detached, Sendable, Swift 6 strict mode (scoped: `**/*.swift`)
- **swift-testing.md** — неочевидные правила Swift Testing: изоляция `@Suite`, `#require` vs `#expect`, fakes вместо mocks (scoped: `**/*.swift`)
- **swiftui-design-system.md** — неочевидные правила дизайн-системы SwiftUI: токены, theming, accessibility-чеклист (scoped: `**/*.swift`)
- **swiftui-patterns.md** — неочевидные структурные паттерны SwiftUI: `AnyView` vs generics, identity view, ViewModifier (scoped: `**/*.swift`)
- **swiftui-performance.md** — неочевидные правила производительности SwiftUI: гранулярность `@Observable`, чистота body (scoped: `**/*.swift`)
- **swiftui-state.md** — неочевидные правила управления state SwiftUI: property wrappers, заморозка `@State` из `init` (scoped: `**/*.swift`)
