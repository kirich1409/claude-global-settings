...
#                                  Capped at 20 to avoid a long sequence of per-issue network calls.
#   -R <owner/repo>                Target repository (default: current repo from git)
#
# All `gh` calls are wrapped in gh_with_timeout (see _common.sh) so a hung API aborts the call.
#
# OUTPUT (stdout, JSON):
...
if [[ -n "$NUMBERS" ]]; then
  ...
  # Cap to avoid an unbounded sequence of per-issue network calls (each one can stall).
  if [[ ${#NUMLIST[@]} -gt 20 ]]; then
    im_error "Too many numbers (max 20): ${#NUMLIST[@]} requested" "usage"
    exit 1
  fi
  results="[]"
  for n in "${NUMLIST[@]}"; do
...
if ! out=$(gh_with_timeout "$GH_REST_TIMEOUT" gh issue list -R "$REPO" \
  --state "$STATE" \
  --limit "$LIMIT" \
