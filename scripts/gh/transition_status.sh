...
#
# MECHANISM:
#   1. Detect GitHub Project v2 by asking the ISSUE which projects it is linked to (1 GraphQL query),
#      then checking those projects only (usually 0-2) — NOT by scanning every project of the owner.
#      If a linked project has a "Status" single-select field, uses GraphQL to set it.
#   2. Fallback: open/closed state + labels (status:in-progress, status:in-review, status:blocked).
#
...
detect_project() {
  ...
  local fields_out
  fields_out=$(gh_with_timeout "$GH_GQL_TIMEOUT" gh api graphql \
  ...
  local items_out
  items_out=$(gh_with_timeout "$GH_GQL_TIMEOUT" gh api graphql \
...
# Find a project — use forced id if provided, else ask the issue which projects it is linked to
# (1 query) and check only those. This avoids scanning every project of the owner (was up to ~61
# sequential calls); now it is O(linked projects), typically 2-3 calls total.
USE_PROJECT=false
...
if [[ -n "$FORCED_PROJECT_ID" ]]; then
  ...
  # Ask the issue for its linked project ids (single query).
  if linked_out=$(gh_with_timeout "$GH_GQL_TIMEOUT" gh api graphql \
    -f query='query($nodeId:ID!){node(id:$nodeId){... on Issue{projectItems(first:10){nodes{project{id}}}}}}' \
    -f nodeId="$ISSUE_NODE_ID" 2>&1); then linked_rc=0; else linked_rc=$?; fi
  if [[ $linked_rc -eq 0 ]] && ! jq -e '.errors' >/dev/null 2>&1 <<<"$linked_out"; then
    proj_ids=$(printf '%s' "$linked_out" | jq -r '.data.node.projectItems.nodes[]?.project.id' 2>/dev/null | sort -u || true)
  ...
  # Scope/permission/no-link errors degrade to labels silently
fi

# Determine current status and mechanism.
# project-v2 is used only when the issue is linked AND the board actually has the target option
# (e.g. a board without an "In Review" column leaves PROJECT_TARGET_OPTION_ID empty → labels).
...
if [[ "$MECHANISM" == "project-v2" ]]; then
  if ! mut_out=$(gh_with_timeout "$GH_GQL_TIMEOUT" gh api graphql \
  ...
  for lbl in "${TARGET_ADD_LABELS[@]+"${TARGET_ADD_LABELS[@]}"}"; do
    ...
    create_out=$(gh_with_timeout "$GH_REST_TIMEOUT" gh label create "$lbl" -R "$IM_REPO" --color "ededed" --description "Issue status: $lbl" 2>&1) || true
    # After attempted create, verify the label actually exists (handles both success and "already exists").
    if ! gh_with_timeout "$GH_REST_TIMEOUT" gh label list -R "$IM_REPO" --search "$lbl" --json name -q '.[].name' 2>/dev/null | grep -qxF "$lbl"; then
      im_error "Failed to ensure label $lbl exists: $create_out" "gh_failed"
      exit 1
    fi
    add_out=$(gh_with_timeout "$GH_REST_TIMEOUT" gh issue edit "$IM_NUMBER" -R "$IM_REPO" --add-label "$lbl" 2>&1) || {
  ...
  if [[ "$TARGET_OPEN" == "false" && "$ISSUE_STATE" == "OPEN" ]]; then
    close_out=$(gh_with_timeout "$GH_REST_TIMEOUT" gh issue close "$IM_NUMBER" -R "$IM_REPO" 2>&1) || {
      im_error "Failed to close issue $IM_NUMBER: $close_out" "gh_failed"
      exit 1
    }
  elif [[ "$TARGET_OPEN" == "true" && "$ISSUE_STATE" == "CLOSED" ]]; then
    reopen_out=$(gh_with_timeout "$GH_REST_TIMEOUT" gh issue reopen "$IM_NUMBER" -R "$IM_REPO" 2>&1) || {
