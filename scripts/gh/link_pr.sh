...
if ! out=$(gh_with_timeout "$GH_REST_TIMEOUT" gh issue view "$ISSUE_NUMBER" -R "$IM_REPO" \
...
if ! new_out=$(gh_with_timeout "$GH_REST_TIMEOUT" gh issue comment "$ISSUE_NUMBER" -R "$IM_REPO" \
...
if ! out2=$(gh_with_timeout "$GH_REST_TIMEOUT" gh issue view "$ISSUE_NUMBER" -R "$IM_REPO" \
  --json comments 2>&1); then
