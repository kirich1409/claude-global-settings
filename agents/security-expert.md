...
- Секреты CI/CD — граница по слою, а не по теме: модель угроз, оценка последствий утечки и требования к обращению с секретом остаются здесь; настройка хранилища, прав доступа и ротации — **devops-expert**. Не переадресовывать ему оценку риска: он вернёт её обратно.
