...
# jq/yq: путь к ключу `.env.FOO` внутри программы неотличим от имени файла по одному
# лишь тексту, поэтому программа исключается из проверки целиком. Программой считается
# одинарно-кавыченный аргумент, который начинается как выражение (`.`, `(`, `[`, `{`)
# и содержит метасимвол jq — то есть `jq '.env.FOO' f.json` (без метасимволов) остаётся
# заблокированным, а `.["env"]` не содержит литерала `.env` и не срабатывает никогда.
# Значения `--arg` и обычные кавыченные имена файлов под исключение не попадают.
...
PATTERNS = [
    # `(` из префикса исключён намеренно: перед именем файла его не бывает — `(cat .env)`
    # даёт пробел, — а вот в выражении вида `(.env.FOO // "x")` он есть всегда.
